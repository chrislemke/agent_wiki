...
2. **Structural pass.**
   ...
   `informational` (backlog, restore, never-cited sources) is reported, not acted on, except that a `restore` finding suggests `/agent-wiki:fetch --restore`. Done when every `judgement` finding is fixed, decided by the owner, or queued for step 4.
